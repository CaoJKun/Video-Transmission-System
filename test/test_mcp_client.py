...
class MCPClient:
    ...
    async def call_tool(self, tool_name: str, arguments: dict, timeout: int = 60):
        ...
        try:
            ...
            if tool_name != "video2pictures_tool" and tool_name != "video2audio_tool" and tool_name != "picturepicker_tool" and tool_name != "video2token_tool" and tool_name != "video2metadata_tool" and tool_name != "video2motionTokens_tool" and tool_name!="upload_video_tool":
    ...
    async def run_with_llm(self, video_path: str, api_key: str, user_prompt: str):
        """
        使用Gemini分析视频内容，然后根据策略调用工具
        """
        logger.info("=== 使用 Gemini 分析视频并执行处理 ===")
        ...
        async with self.client:
            ...
            if not session_id:
                logger.error("上传失败，无法继续")
                return

            logger.info(f"会话 session_id = {session_id}")


            # 使用Gemini分析视频
            result = call_gemini(video_path, user_prompt, api_key, session_id)
            
            # 提取 llm_output
            llm_output = result.get("llm_output", {})
            logger.info(f"Gemini 生成的完整输出: {json.dumps(result, indent=2, ensure_ascii=False)}")
            
            # 显示生成策略和提示词
            generate_policy = llm_output.get("generate_policy", {})
            prompt_generation = llm_output.get("prompt_generation", {})
            
            logger.info("=== 生成策略 ===")
            logger.info(f"模型: {generate_policy.get('model_name', 'N/A')}")
            logger.info(f"模块: {generate_policy.get('module', 'N/A')}")
            logger.info(f"推理: {generate_policy.get('reasoning', 'N/A')}")
            logger.info(f"预期输入: {generate_policy.get('expected_inputs', [])}")
            
            logger.info("=== 生成的提示词 ===")
            logger.info(f"提示词: {prompt_generation.get('prompt', 'N/A')}")
            ...
            # 根据策略执行工具
            process_policy = llm_output.get("process_policy", {})
            
            # 执行各个处理工具
            if process_policy.get("video2pictures"):
                logger.info("执行 video2pictures_tool...")
                await self.call_tool("video2pictures_tool", {"session_id": session_id, "max_frames": 500})
            
            if process_policy.get("picturepicker"):
                logger.info("执行 picturepicker_tool...")
                await self.call_tool("picturepicker_tool", {"session_id": session_id, "max_keyframes": 16})
            
            if process_policy.get("video2audio"):
                logger.info("执行 video2audio_tool...")
                await self.call_tool("video2audio_tool", {"session_id": session_id})
            
            if process_policy.get("audio2text"):
                logger.info("执行 audio2text_tool...")
                await self.call_tool("audio2text_tool", {"session_id": session_id})
            
            if process_policy.get("video2metadata"):
                logger.info("执行 video2metadata_tool...")
                await self.call_tool("video2metadata_tool", {"session_id": session_id})
            
            if process_policy.get("video2motionTokens"):
                logger.info("执行 video2motionTokens_tool...")
                await self.call_tool("video2motionTokens_tool", {"session_id": session_id})

            # 记录最终策略信息
            logger.info("=== 最终处理策略总结 ===")
            enabled_tools = [tool for tool, enabled in process_policy.items() if enabled]
            logger.info(f"启用的工具: {enabled_tools}")
            logger.info(f"生成模型: {generate_policy.get('model_name', 'N/A')}")
            logger.info(f"视频描述: {prompt_generation.get('prompt', 'N/A')}")

        logger.info("=== Gemini 视频分析执行完成 ===")
    ...
    async def run_comprehensive_test(self, video_path: str = None):
        ...
        async with self.client:
            ...
            # 调用 pictures2video_from_session_tool
            logger.info("调用 pictures2video_from_session_tool 从会话帧数据生成视频")
            ...
            if frames2video_result and frames2video_result.data and frames2video_result.data.get("status") == "success":
                ...
                logger.info(f"帧转视频成功，输出路径: {output_path}")
                ...
                logger.info(f"处理信息: 输入帧数={processing_info.get('input_frames', 'N/A')}, 处理帧数={processing_info.get('processed_frames', 'N/A')}, 编码器={processing_info.get('codec', 'N/A')}")
            else:
                err_msg = frames2video_result.data.get("message") if (frames2video_result and frames2video_result.data) else "未知错误"
                logger.error(f"帧转视频失败: {err_msg}")
                return False
...
async def main():
    ...
    # 新增参数
    parser.add_argument("--llm", action="store_true", help="启用 LLM 决策模式")
    parser.add_argument("--prompt", default="请根据用户需求输出视频处理计划", help="传给 LLM 的提示词")
    parser.add_argument("--api_key", default=os.environ.get("填写gemini的api-key"), help="Gemini API Key")
    ...
    try:
        if args.llm:
            await client.run_with_llm(args.video, args.api_key, args.prompt)
            logger.info("LLM 决策模式执行完成！")
        else:
